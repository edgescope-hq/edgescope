          ...
          <div className="w-full max-w-6xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-2 flex items-center justify-between">
              <div className="text-xs font-semibold tracking-wider text-muted-foreground">
                FULLSCREEN ANNOTATOR — {kind.toUpperCase()}
              </div>
              <button
                onClick={() => setFullscreen(false)}
...
function AnnotatorCanvas({
  ...
  useEffect(() => {
    setShapes(initial ?? []);
  }, [screenshotId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2 rounded-lg bg-white/[0.03] p-2 ring-1 ring-white/[0.04]">
        ...
        {COLORS.map((c) => (
          <button
            ...
            aria-label={`Color ${c}`}
            className={cn(
              "h-5 w-5 rounded-full ring-1 ring-white/20",
        ...
        <div className="ml-auto flex items-center gap-1">
          <button
            ...
            title="Undo"
            aria-label="Undo"
            className="grid h-8 w-8 place-items-center rounded-md bg-white/[0.04] text-muted-foreground ring-1 ring-white/[0.06] hover:text-foreground"
          >
          ...
          <button
            ...
            className="grid h-8 w-8 place-items-center rounded-md bg-white/[0.04] text-muted-foreground ring-1 ring-white/[0.06] hover:text-destructive"
          >
          ...
          <button
            ...
            className="inline-flex h-8 items-center gap-1.5 rounded-md bg-primary px-2.5 text-[11px] font-semibold text-primary-foreground hover:brightness-110 disabled:opacity-50"
          >
      ...
      <div
        className={cn(
          "relative overflow-hidden rounded-lg ring-1 ring-white/[0.06]",
          compact ? "max-h-[420px]" : "max-h-[80vh]",
        )}
      >
      ...
      return (
        <g key={s.id}>
          <line
            ...
            y2={s.y2}
            {...stroke}
            markerEnd={`url(#arr-${c.replace("#", "")})`}
          />
          {labelText && (
            <text x={s.x2 + 6} y={s.y2 - 6} fill={c} fontSize="22" fontWeight="700">
              {labelText}
            </text>
          )}
        </g>
    ...
    case "rect": {
      ...
      return (
        <g key={s.id}>
          ...
          {labelText && (
            <text x={x + 4} y={y - 6} fill={c} fontSize="20" fontWeight="700">
              {labelText}
            </text>
          )}
        </g>
    ...
    case "circle": {
      ...
      return (
        <g key={s.id}>
          ...
          {labelText && (
            <text x={cx + rx + 4} y={cy} fill={c} fontSize="20" fontWeight="700">
              {labelText}
            </text>
          )}
        </g>
