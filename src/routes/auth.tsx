...
function SignUpForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      // If signup did not return a session, try signing in directly.
      // First admin can be auto-confirmed by the database trigger.
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <OAuthDivider />
      {needsBootstrap === true && (
        <div className="rounded-xl border border-primary/25 bg-primary/10 px-3 py-2 text-xs leading-5 text-muted-foreground">
          You are the first user. This account will be created as the admin.
        </div>
      )}
      <div className="space-y-2">
        <Label
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          className="h-11 rounded-xl border-white/[0.08] bg-white/[0.045] text-foreground transition-colors duration-200 placeholder:text-muted-foreground/50 focus:border-primary/50 focus:ring-primary/25"
        />
        <p className="text-xs text-muted-foreground/70">Use at least 8 characters.</p>
      </div>
      <Button
        type="submit"
        className="h-11 w-full rounded-xl bg-primary text-sm font-semibold shadow-[var(--shadow-glow)] transition-all duration-200 hover:-translate-y-px hover:brightness-110"
